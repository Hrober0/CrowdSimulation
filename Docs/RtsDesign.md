...
**Buildings can be turned before they are placed, and the preview draws the door.** `BuildingPlacement.Rotation` existed from step 5 and `BuildingFootprintSystem` had always honoured it, for both the footprint and the entrances — but nothing could set it. Every building went down facing south, so on a map with one-way roads the only way to get a door onto the right side of a building was to put the building somewhere else. `R` turns it a quarter turn clockwise, and the rotation is kept across placements, because someone laying a row of huts along a road wants them all facing the road.

Two things this exposed are worth recording:

- **The placement check had drifted from the system it was checking for.** `CanPlace` walked an unrotated `Size` rectangle and hard-coded the doorstep as "one below the origin", while the system rotated both. Unrotated that agrees by accident; the moment rotation is settable it approves a building onto ground it will not occupy and checks a doorstep it will not have. The transform now lives once, in `BuildingGeometry` in the `Rts` assembly, and the system and the check both call it. That it was *also* the only way to test the geometry — `RtsTests` cannot see the examples assembly, and adding the reference is not allowed — is the asmdef graph doing its job: the rule about where a building's cells land is a building rule, and it was in the wrong module.
- **The preview has to draw cell by cell, not as one box.** Rotation turns the shape around the origin cell rather than around its middle, so a turned building extends a different way from the cursor. A bounding box derived from `Size` keeps showing the unrotated footprint — a preview that lies about where the thing is going.

The door is drawn because the door is the half that decides. A building is a blocked rectangle wherever it goes; what makes one placement work and another useless is which way the single doorstep faces and whether anything can reach it. Until now that was invisible until the building existed, so the only way to find out was to build it and look. The marker is amber when the doorstep is walkable and red when it is not — a sealed doorway being the one placement mistake that produces a finished building nobody can ever use.
