...
namespace AgentSimulation
{
    ...
    public struct UpdateAgentJobParallel : IJobParallelFor
    {
        ...
        public void Execute(int index)
        {
            ...
            ComputeAgentNeighbor(_currentAgent.NeighborDist, agentNeighbors);
            
            // Compute Obstacle Neighbor
            var obstacleNeighbors = new NativeList<ObstacleVertexNeighbor>(Allocator.Temp);
            ComputeObstacleNeighbor(_currentAgent.TimeHorizonObstacle * _currentAgent.MaxSpeed + _currentAgent.Radius, obstacleNeighbors);
            ...
            int lineFail = LinearProgram2(orcaLines, _currentAgent.MaxSpeed, _currentAgent.PrefVelocity, false, ref newVelocity);
            if (lineFail < orcaLines.Length)
            {
                LinearProgram3(orcaLines, numObstLines, lineFail, _currentAgent.MaxSpeed, ref newVelocity);
            }


            var velocityLengthSqr = math.lengthsq(newVelocity);
            if (velocityLengthSqr > _currentAgent.MaxSpeed)
            {
                newVelocity = newVelocity / velocityLengthSqr * _currentAgent.MaxSpeed;
            }
            _currentAgent.Velocity = newVelocity;
            //_currentAgent.Position += newVelocity * TimeStamp;

            Agents[index] = _currentAgent;
        ...
        private void ComputeAgentNeighbor(float range, NativeList<AgentNeighbor> agentNeighbors)
        {
            ...
            var position = _currentAgent.Position;
        ...
        {
            if (_currentAgent.ObjectId == neighbor.ObjectId)
            {
                return;
            }

            float distSq = math.lengthsq(_currentAgent.Position - neighbor.Position);
            ...
            if (agentNeighbors.Length < _currentAgent.MaxNeighbors)
            {
            ...
            for (int y = minY; y <= maxY; y++)
            {
                for (int x = minX; x <= maxX; x++)
                {
                    foreach (var vertexIndex in ObstacleVerticesLookup.GetValuesForKey(RVOMath.ChunkHash(x, y)))
                    {
                        ...
                        float agentLeftOfLine = RVOMath.LeftOf(startPosition, endPosition, _currentAgent.Position);
                        ...
                        if (distSqLine < rangeSq)
                        {
                            ...
                            InsertObstacleNeighbor(vertex, obstacleNeighbors, rangeSq);
                        }
                    }
                }
            }
        }
        private void InsertObstacleNeighbor(ObstacleVertex obstacle, NativeList<ObstacleVertexNeighbor> obstacleNeighbors, float rangeSq)
        {
            var nextObstacle = ObstacleVertices[obstacle.Next];

            float distSq = RVOMath.DistSqPointLineSegment(obstacle.Point, nextObstacle.Point, _currentAgent.Position);
        ...
        private readonly void AddAgentLine(NativeList<Line> orcaLines, NativeList<AgentNeighbor> agentNeighbors, float timeStampInv)
        {
            ...
            for (int i = 0; i < agentNeighbors.Length; ++i)
            {
                ...
                float2 relativePosition = other.Position - _currentAgent.Position;
                float2 relativeVelocity = _currentAgent.Velocity - other.Velocity;
                float distSq = math.lengthsq(relativePosition);
                float combinedRadius = _currentAgent.Radius + other.Radius;
        ...
        private readonly void AddObstacleLine(NativeList<Line> orcaLines, NativeList<ObstacleVertexNeighbor> obstacleNeighbors, NativeList<ObstacleVertex> obstacles)
        {
            float invTimeHorizonObst = 1f / _currentAgent.TimeHorizonObstacle;
            float radius = _currentAgent.Radius;
            float2 velocity = _currentAgent.Velocity;
            float2 position = _currentAgent.Position;
            ...
            for (int i = 0; i < obstacleNeighbors.Length; ++i)
            {
                ...
                float2 relativePosition1 = obstacle1.Point - _currentAgent.Position;
                float2 relativePosition2 = obstacle2.Point - _currentAgent.Position;
        ...
        private readonly bool LinearProgram1(NativeList<Line> lines, int lineNo, float radius, float2 optVelocity, bool directionOpt, ref float2 result)
        {
        ...
        private readonly int LinearProgram2(NativeList<Line> lines, float radius, float2 optVelocity, bool directionOpt, ref float2 result)
        {
            if (directionOpt)
            {
                /*
        ...
        private readonly void LinearProgram3(NativeList<Line> lines, int numObstLines, int beginLine, float radius, ref float2 result)
        {
            float distance = 0;
        ...
        private struct ObstacleVertexNeighbor
        {
            public float Distance;
            public ObstacleVertex Obstacle;
        }

        private struct AgentNeighbor
        {
            public float Distance;
            public Agent Agent;
        }
